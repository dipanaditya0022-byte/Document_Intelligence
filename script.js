...
function saveConfig() {
  localStorage.setItem(
    STORAGE_KEY,
    JSON.stringify({
      modelId: modelIdEl.value,
      locale: localeEl.value.trim(),
      pages: pagesEl.value.trim(),
      stringIndexType: stringIndexTypeEl.value
    })
  );
...
function updateSourceSummary() {
  ...
  if (activeSourcePane === "uploadPane") {
    ...
    heroSource.textContent = val ? "Public URL" : "URL mode";
    topbarSource.textContent = val ? "URL" : "URL mode";
...
function resetResults() {
  lastResult = null;
  textOutput.textContent = "No text yet.";
  splitTextOutput.textContent = "No text extracted yet.";
  ...
  ovPages.textContent = "—";
  ovWords.textContent = "—";
  ovTables.textContent = "—";
  ovDocs.textContent = "—";
  resultSummary.textContent = "No result yet";
...
function clearAll() {
  ...
  docPreviewArea.innerHTML = `<div class="doc-preview-empty"><div class="icon">📁</div>Upload a document to preview it here</div>`;
  resetResults();
  setStatus("Idle", "warn", "Waiting for input");
  heroSource.textContent = "No source selected";
  topbarSource.textContent = "No source";
  inputSummary.textContent = "No input selected";
  updateSourceSummary();
}

function loadHistory() {
  try {
    analysisHistory = JSON.parse(localStorage.getItem(HISTORY_KEY) || "[]");
  } catch {
    analysisHistory = [];
  }
}
...
function renderHistory() {
  if (!analysisHistory.length) {
    historyList.innerHTML = `<div class="placeholder" style="min-height:70px;font-size:11px;"><div class="pl-icon" style="font-size:22px;">📂</div>No history yet</div>`;
    return;
  }
  ...
  historyList.querySelectorAll(".history-item").forEach(el => {
    el.addEventListener("click", () => {
      ...
      if (entry?.result) {
        ...
        renderResult(entry.result, entry.model);
        setStatus("Loaded", "good", `Loaded: ${entry.name}`);
        setProgress(100);
...
function getCurrentSource() {
  ...
  if (activeSourcePane === "pastePane" && pastedImageBlob) {
    return {
      type: "file",
      file: new File([pastedImageBlob], pastedImageName || "pasted.png", {
        type: pastedImageBlob.type || "image/png"
      })
    };
  }
  return null;
...
async function analyzeDocument() {
  ...
  const locale = localeEl.value.trim();
  const pages = pagesEl.value.trim();
  const stringIndexType = stringIndexTypeEl.value.trim();

  const source = getCurrentSource();
  if (!source) {
    alert("Please provide a source: upload file, public URL, or pasted image.");
    return;
  }

  analyzeBtn.disabled = true;
  resetResults();
  setStatus("Submitting", "warn", "Sending request securely via Vercel...");
  setProgress(12);
  ...
  try {
    ...
    setStatus("Processing", "warn", "Azure is analyzing...");
    setProgress(32);

    const result = await submitAnalyzeRequest({
      modelId,
      locale,
      pages,
      stringIndexType,
    ...
    lastResult = result;
    renderResult(result, modelId);
    setStatus("Succeeded", "good", "Analysis completed! 🎉");
    setProgress(100);
    copyTextBtn.disabled = false;
    downloadJsonBtn.disabled = false;
    ...
    console.error(err);
    setStatus("Failed", "bad", err.message || "Something went wrong");
    setProgress(100);
...
async function testConnection() {
  setConnStatus("testing", "Testing connection...");
  setStatus("Testing", "warn", "Checking Azure connection...");
  setProgress(20);
  ...
  try {
    ...
    if (!res.ok) {
      setConnStatus("bad", `Failed (${res.status})`);
      setStatus("Failed", "bad", `Connection failed (${res.status})`);
      jsonOutput.textContent = JSON.stringify(data, null, 2);
      setProgress(100);
      return;
    }

    setConnStatus("good", "Connected ✓");
    setStatus("Connected ✓", "good", "Azure resource is reachable!");
    jsonOutput.textContent = JSON.stringify(data, null, 2);
    setProgress(100);
  } catch (err) {
    setConnStatus("bad", "Connection failed");
    setStatus("Failed", "bad", err.message || "Connection test failed");
    jsonOutput.textContent = JSON.stringify({ error: err.message || String(err) }, null, 2);
    setProgress(100);
  }
}
...
function renderResult(result, selectedModelId) {
  ...
  splitTextOutput.textContent = text || "No text extracted.";
  metaPages.textContent = pages.length || "0";
  metaModel.textContent = ar.modelId || selectedModelId || "—";
  heroModel.textContent = ar.modelId || selectedModelId || "—";
  topbarModel.textContent = ar.modelId || selectedModelId || "—";
  resultSummary.textContent = `Pages: ${pages.length || 0} · Tables: ${tables.length || 0} · Docs: ${documents.length || 0}`;
  statPages.textContent = pages.length || "0";
  statWords.textContent = words.toLocaleString();
  statTables.textContent = tables.length || "0";
  ovPages.textContent = pages.length || "0";
  ovWords.textContent = words.toLocaleString();
  ovTables.textContent = tables.length || "0";
  ovDocs.textContent = documents.length || "0";

  renderOverviewDetails(ar, documents, selectedModelId);
  renderFields(documents);
  renderTables(tables);
}
  ...
  documents.forEach(doc => {
    Object.entries(doc.fields || {}).forEach(([name, field]) => {
      if (typeof field?.confidence === "number") {
        allFields.push({ name, confidence: field.confidence });
      }
    });
  });

  const confBars = allFields.length
    ? `<div class="card" style="margin-top:14px;"><div class="card-title">📈 Confidence Scores</div><div class="conf-chart">${allFields
        .slice(0, 12)
        .map(
          f => `<div class="conf-bar-row"><div class="conf-label">${escapeHtml(f.name)}</div><div class="conf-track"><div class="conf-fill" style="width:${(f.confidence * 100).toFixed(1)}%"></div></div><div class="conf-val">${(f.confidence * 100).toFixed(0)}%</div></div>`
        )
        .join("")}</div></div>`
    : "";
...
function renderFields(documents) {
  if (!documents.length) {
    fieldsOutput.innerHTML = `<div class="placeholder"><div class="pl-icon">🏷</div>No structured fields for this model.</div>`;
    return;
  }
  ...
  chunks.push("</div>");
  fieldsOutput.innerHTML = chunks.join("");
}

function renderTables(tables) {
  if (!tables.length) {
    tablesOutput.innerHTML = `<div class="placeholder"><div class="pl-icon">📋</div>No tables detected.</div>`;
    return;
  }
  ...
  tables.forEach((table, idx) => {
    ...
    (table.cells || []).forEach(cell => {
      ...
      if (r < rows && c < cols) grid[r][c] = cell.content || "";
    });
...
async function copyExtractedText() {
  ...
  try {
    ...
    setStatus("Copy failed", "bad", "Clipboard permission denied");
  }
}
...
function downloadJson() {
  ...
  a.href = url;
  a.download = "azure-docintel-result.json";
  a.click();
